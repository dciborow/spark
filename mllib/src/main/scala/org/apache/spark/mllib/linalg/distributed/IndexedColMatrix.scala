...
/**
  * Represents a row of [[org.apache.spark.mllib.linalg.distributed.IndexedColMatrix]].
  */
@Since("1.0.0")
case class IndexedCol(index: Long, vector: Vector)
...
/**
  ...
  */
@Since("1.0.0")
class IndexedColMatrix @Since("1.0.0")(
  @Since("1.0.0") val rows: RDD[IndexedCol],
  private var nRows: Long,
  private var nCols: Int) extends DistributedMatrix {

  /** Alternative constructor leaving matrix dimensions to be determined automatically. */
  @Since("1.0.0")
  def this(rows: RDD[IndexedCol]) = this(rows, 0L, 0)
  ...
  def toBlockMatrix(rowsPerBlock: Int, colsPerBlock: Int): BlockMatrix = {
    ...
    val blocks = rows.flatMap { ir: IndexedCol =>
  ...
  def toCoordinateMatrix(): CoordinateMatrix = {
    val entries = rows.flatMap { row =>
      ...
      row.vector match {
        case SparseVector(size, indices, values) =>
          Iterator.tabulate(indices.length)(i => MatrixEntry(rowIndex, indices(i).toLong, values(i)))
        case DenseVector(values) =>
          Iterator.tabulate(values.length)(i => MatrixEntry(rowIndex, i.toLong, values(i)))
      }
    }
  ...
  /**
    * Computes the singular value decomposition of this IndexedColMatrix.
    ...
    *
    * U is an [[org.apache.spark.mllib.linalg.distributed.IndexedColMatrix]] of size m x k that
    * satisfies U'U = eye(k),
  ...
  def computeSVD(
    ...
    rCond: Double = 1e-9): SingularValueDecomposition[IndexedColMatrix, Matrix] = {
    ...
    val U = if (computeU) {
      val IndexedCols = indices.zip(svd.U.rows).map { case (i, v) =>
        IndexedCol(i, v)
      }
      new IndexedColMatrix(IndexedCols, nRows, svd.U.numCols().toInt)
    } else {
      null
    }
  ...
  /**
    ...
    * @return an IndexedColMatrix representing the product, which preserves partitioning
    */
  @Since("1.0.0")
  def multiply(B: Matrix): IndexedColMatrix = {
    val mat = toRowMatrix().multiply(B)
    val IndexedCols = rows.map(_.index).zip(mat.rows).map { case (i, v) =>
      IndexedCol(i, v)
    }
    new IndexedColMatrix(IndexedCols, nRows, B.numCols)
  }
  ...
  private[mllib] override def toBreeze(): BDM[Double] = {
    ...
    rows.collect().foreach { case IndexedCol(rowIndex, vector) =>
