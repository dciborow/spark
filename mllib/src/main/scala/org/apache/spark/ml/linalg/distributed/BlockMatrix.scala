...
import org.apache.spark.ml.linalg.{DenseMatrix, Matrices, Matrix, SparseMatrix, Vectors}
...
class BlockMatrix(val blocks: Dataset[((Int, Int), Matrix)],
  ...
  def toCoordinateMatrix(): CoordinateMatrix = {
    val entryRDD = blocks.rdd.flatMap { case ((blockRowIndex, blockColIndex), mat) =>
    ...
    new CoordinateMatrix(entryRDD.toDS(), numRows(), numCols())
  }
  ...
  def toIndexedRowMatrix(): IndexedRowMatrix = {
    ...
    val rows = blocks.rdd.flatMap { case ((blockRowIdx, blockColIdx), mat) =>
  ...
  def transpose: BlockMatrix = {
    import blocks.sparkSession.implicits._
    val transposedBlocks = blocks.rdd.map { case ((blockRowIndex, blockColIndex), mat) =>
      ((blockColIndex, blockRowIndex), mat.transpose)
    }
    new BlockMatrix(transposedBlocks.toDS(), colsPerBlock, rowsPerBlock, nCols, nRows)
  }
  ...
  private[ml] def blockMap(
    ...
    if (rowsPerBlock == other.rowsPerBlock && colsPerBlock == other.colsPerBlock) {
      val newBlocks = blocks.rdd.cogroup(other.blocks.rdd, createPartitioner())
  ...
  def multiply(
    ...
    if (colsPerBlock == other.rowsPerBlock) {
      ...
      val flatA = blocks.rdd.flatMap { case ((blockRowIndex, blockColIndex), block) =>
      ...
      val flatB = other.blocks.rdd.flatMap { case ((blockRowIndex, blockColIndex), block) =>
      ...
      import blocks.sparkSession.implicits._
      new BlockMatrix(newBlocks.toDS(), rowsPerBlock, other.colsPerBlock, numRows(), other.numCols())
    } else {
